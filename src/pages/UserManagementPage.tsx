...
import { UserPlus, Search, X, Pencil, Trash2, MoreHorizontal, ShieldCheck } from 'lucide-react'
...
import { MOCK_USERS, type CRMUser } from '@/constants/mockData'
...
function useUsersState() {
  const [users, setUsers] = useState<CRMUser[]>(MOCK_USERS)

  const invite = useCallback((u: CRMUser) => {
    setUsers(prev => [u, ...prev])
    toast.success(`Invitation sent to ${u.email}`)
  }, [])

  const update = useCallback((updated: CRMUser) => {
    setUsers(prev => prev.map(u => u.id === updated.id ? updated : u))
  }, [])

  const toggleActive = useCallback((id: string, currentId: string) => {
    setUsers(prev => prev.map(u => {
      if (u.id !== id) return u
      if (u.id === currentId) { toast.error('You cannot deactivate your own account'); return u }
  ...
  const remove = useCallback((id: string, currentId: string) => {
    if (id === currentId) { toast.error('You cannot delete your own account'); return }
    setUsers(prev => prev.filter(u => u.id !== id))
    toast.success('User deleted')
  }, [])

  return { users, invite, update, toggleActive, remove }
}

export function UserManagementPage() {
  const { user: me, hasPermission } = useAuth()
  const { users, invite, update, toggleActive, remove } = useUsersState()
  const [tab, setTab]               = useState<Tab>('users')
  const [search, setSearch]         = useState('')
  const [roleFilter, setRoleFilter] = useState<string>('all')
  const [inviteOpen, setInviteOpen] = useState(false)
  const [editing, setEditing]       = useState<CRMUser | null>(null)
  const [deleting, setDeleting]     = useState<CRMUser | null>(null)
    ...
    <>
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            {canEdit ? (
                              <Switch
                                checked={u.is_active}
                                onCheckedChange={() => toggleActive(u.id, me?.id ?? '')}
                              ...
                              <>
                                ...
                                <div className="absolute right-0 top-8 z-20 w-36 bg-card border border-border rounded-xl shadow-xl overflow-hidden py-1">
                                  ...
                                  <PermissionGate permission="users_delete">
                                    ...
                                    <button type="button"
                                      onClick={() => { isSelf ? toast.error('You cannot delete your own account') : setDeleting(u); setOpenMenu(null) }}
                                      disabled={isSelf}
                                      className="w-full flex items-center gap-2 px-3 py-2 text-sm text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 disabled:opacity-40 transition-colors">
      ...
      {editing && (
        <EditUserModal
          ...
          onSave={u => { update(u); setEditing(null) }}
        />
      )}
      ...
      {deleting && (
        <DeleteConfirm
          ...
          onConfirm={() => { remove(deleting.id, me?.id ?? ''); setDeleting(null) }}
